...
void XHMM::DiscoverOutputManager::printCNV(SampleGenotypeQualsCalculator* genotyper, const uint t1, const uint t2, const uint type, HMM_PP::Data* origData) {
	Genotype* gt = new Genotype(genotyper, t1, t2, CNVmodelParams::DIPLOID, type);
	...
	if (_discoverSomeQualThreshold.passScore(typeAllele->getHaveSomeEventScore())) {
		...
		<< '\t' << gt->getMergedTargets()
		<< '\t' << gt->getMergedTargets().spanKB()
		<< '\t' << gt->getMergedTargets().getChr()
		<< '\t' << gt->getMergedTargets().midpoint()
		<< '\t' << gt->getTargetRange().getOneBasedTargetIndexSpan()
		<< '\t' << gt->getTargetRange().getNumTargets()
