...
void XHMM::PCA_NormalizeOutputManager::PCA(string rdFile) const {
	...
	HMM_PP::MatrixDecomp::SVDecomp decomp = HMM_PP::MatrixDecomp::svd(rdMat, true, _saveMemoryDir); // get V-transpose as V
#if defined(DEBUG)
	HMM_PP::DoubleMat* sigma = decomp.D->diag();
	HMM_PP::DoubleMat* sigmaVtranspose = *sigma * *decomp.V;
	delete sigma;
	HMM_PP::DoubleMat* testMat = *decomp.U * *sigmaVtranspose;
	delete sigmaVtranspose;
	HMM_PP::DoubleMat* diff = *testMat - *rdMat;
	delete testMat;
