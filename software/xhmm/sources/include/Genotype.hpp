...
namespace XHMM {
	...
	public:
		...
		Genotype(const SampleGenotypeQualsCalculator* genotypeCalc, const uint t1, const uint t2, const uint refType, const vector<uint>& nonRefTypes, const HMM_PP::ModelParams::DataVal* meanOrigRD = NULL, const RealThresh* callQualThresh = NULL);
		Genotype(const SampleGenotypeQualsCalculator* genotypeCalc, const uint t1, const uint t2, const uint refType, const uint nonRefType, const HMM_PP::ModelParams::DataVal* meanOrigRD = NULL, const RealThresh* callQualThresh = NULL);
		...
		BaseReal getNonDiploidScore() const { return _nonDiploidScore; }
		BaseReal getDiploidScore() const { return _diploidScore; }

		uint getNumNonRefAlleles() const { return _nonRefAlleles->size(); }
		const AlleleQuals* getNonRefAllele(uint type) const { return (*_nonRefAlleles)[type]; }
		const map<uint, AlleleQuals*>* getNonRefAlleles() const { return _nonRefAlleles; }

		BaseReal getPL(const uint type) const;
	...
	private:
		...
		void createNonRefAllelesAndCallGenotype(const SampleGenotypeQualsCalculator* genotypeCalc, const uint t1, const uint t2, const uint refType, const vector<uint>& nonRefTypes, const RealThresh* callQualThresh);
	};
