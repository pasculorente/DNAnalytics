...
namespace HMM_PP {
	...
	public:
		...
		virtual double transitionFunc(const uint i, const uint j, const uint t1, const uint t2) const = 0;

		virtual double emissionFunc(const HMM_PP::Data* sequence, const uint i, const uint t) const = 0;
		...
		virtual DoubleVec getVariableParams() const = 0;
		virtual void setVariableParams(const DoubleVec& v) = 0;
		...
		const DoubleVec& getInitProbs() const { return _params->_initProbs; }
		void setStartingProbsAndNormalize(const DoubleVec& x) { _params->setStartingProbsAndNormalize(x); }
	...
	protected:
		...
		static double dnorm(const double x, const double u, const double v);
		static double univarNorm(const double x, const double u, const double s);
		...
		public:
			...
			DoubleVec getUniformStartProbs() const;
			void setStartingProbsAndNormalize(const DoubleVec& x);
		};
