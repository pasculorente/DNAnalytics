...
double HMM_PP::Model::transitionFunc(const uint i, const uint j, const uint t1, const uint t2) const {
...
double HMM_PP::Model::emissionFunc(const Data* seq, const uint hidden_state, const uint t) const {
	return _modelParams->emissionFunc(seq, hidden_state, t);
}

vector<double>* HMM_PP::Model::emissionFunc(const HMM_PP::Data* seq, const uint t) const {
...
double f_simplex(HMM_PP::DoubleVec& v, void* d) {
...
void HMM_PP::Model::reestimateInitProbs() {
	DoubleVec initProbs(_modelParams->getNumHiddenStates(), 0);
