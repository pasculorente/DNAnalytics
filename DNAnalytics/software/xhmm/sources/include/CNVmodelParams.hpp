...
namespace XHMM {
	...
	public:
		// Updates
		virtual HMM_PP::DoubleVec getVariableParams() const;
		virtual void setVariableParams(const HMM_PP::DoubleVec& v);

		// Get/calculate values
		virtual double transitionFunc(const uint i, const uint j, const uint t1, const uint t2) const;
	protected:
		double transitionProb(const uint i, const uint j, const uint distInBases) const;
	public:
		...
		virtual void scale(const double n) { throw new Exception("should not be used"); }
	...
	private:
		...
		double _p;      // Probability of starting a del CNV = Prob. of starting a dup CNV
		double _e;      // Expected CNV length (# of targets)
		double _d;      // Expected inter-target distance within CNVs
		double _lambda; // Parameter for exponential decay of inter-target distance distribution [ == 1 / _d ]

		// _p, _e, _d:
		static const uint NUM_TRANS_MAT_PARAMS_TO_READ = 3;

		inline const HMM_PP::DoubleVec& getDiploidTransProbs() const {
