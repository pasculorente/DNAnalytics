...
HMM_PP::DoubleVec HMM_PP::UnivariateCategoricalModelParams::getVariableParams() const {
	DoubleVec v;
	...
	for (uint i = 0; i < _params->getNumHiddenStates(); i++)
		for (uint j = 0; j < _numObservedStates-1; j++)
			v.addElement(_emissionMat(i,j));

	return v;
}

void HMM_PP::UnivariateCategoricalModelParams::setVariableParams(const DoubleVec& v) {
	uint vInd = 0;
	...
	_emissionMat.setDims(_params->getNumHiddenStates(), _numObservedStates, 1/(double)_numObservedStates);
...
double HMM_PP::UnivariateCategoricalModelParams::emissionFunc(const HMM_PP::Data* sequence, const uint i, const uint t) const {
	const DataType* d = castData(sequence);
	return EMISS_FUNC;
}

vector<double>* HMM_PP::UnivariateCategoricalModelParams::emissionFunc(const HMM_PP::Data* sequence, const uint t) const {
	const DataType* d = castData(sequence);
...
void HMM_PP::UnivariateCategoricalModelParams::scale(const double n) {
