...
HMM_PP::DoubleVec HMM_PP::HomogeneousModelParams::getHomogeneousStationaryProbs() const {
	HMM_PP::DoubleMat* transMatTranspose = _transitionMat.transpose();
	HMM_PP::MatrixDecomp::EigenDecomp decomp = HMM_PP::MatrixDecomp::rightEigen(transMatTranspose);
	if (!PreciseNonNegativeReal<double>::equalsReal((*decomp.eigenValsReal)[0], 1.0) || !PreciseNonNegativeReal<double>::equalsReal((*decomp.eigenValsImaginary)[0], 0.0))
		throw new Exception("Transition matrix must be stochastic [hence with largest eigenvalue of 1]");

	HMM_PP::DoubleMat* eigenVectorsAsRows = decomp.eigenVectors->transpose();
	decomp.deleteData();
	HMM_PP::DoubleVec initProbs((*eigenVectorsAsRows)[0]);
	delete eigenVectorsAsRows;

	return initProbs.normalize();
}

void HMM_PP::HomogeneousModelParams::readTransMatParams(istream& stream, const uint numHiddenStates, DoubleVec& paramValues) {
...
void HMM_PP::HomogeneousModelParams::getTransMatParams(DoubleVec& v) const {
	for (uint i = 0; i < _params->getNumHiddenStates(); i++)
		for (uint j = 0; j < _params->getNumHiddenStates()-1; j++)
			v.addElement(_transitionMat(i,j));
}

void HMM_PP::HomogeneousModelParams::setTransMatFromParams(const DoubleVec& v, uint& vInd) {
	...
	_transitionMat.setDims(_params->getNumHiddenStates(), _params->getNumHiddenStates(), 1/(double)_params->getNumHiddenStates());
...
double HMM_PP::HomogeneousModelParams::transitionFunc(const uint i, const uint j, const uint t1, const uint t2) const {
...
void HMM_PP::HomogeneousModelParams::scale(const double n) {
	_transitionMat /= n;
}
