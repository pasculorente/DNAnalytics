...
void HMM_PP::UnivariateQuantitativeModelParams::readGaussianParams(istream& stream, const uint numHiddenStates, DoubleVec& paramValues) {
...
void HMM_PP::UnivariateQuantitativeModelParams::getGaussianParams(DoubleVec& v) const {
...
void HMM_PP::UnivariateQuantitativeModelParams::setGaussianFromParams(const DoubleVec& v, uint& vInd) {
	...
	for (uint i = 0; i < _params->getNumHiddenStates(); i++) {
		...
		if (_var[i] <= 0)
			_var[i] = PreciseNonNegativeReal<double>::REAL_EPSILON;
	}
}

HMM_PP::DoubleVec HMM_PP::UnivariateQuantitativeModelParams::getVariableParams() const {
	DoubleVec v;
...
void HMM_PP::UnivariateQuantitativeModelParams::setVariableParams(const DoubleVec& v) {
	uint vInd = 0;
...
double HMM_PP::UnivariateQuantitativeModelParams::emissionFunc(const HMM_PP::Data* sequence, const uint i, const uint t) const {
	const DataType* d = castData(sequence);
	return EMISS_FUNC;
}

vector<double>* HMM_PP::UnivariateQuantitativeModelParams::emissionFunc(const HMM_PP::Data* sequence, const uint t) const {
	const DataType* d = castData(sequence);
...
void HMM_PP::UnivariateQuantitativeModelParams::scale(const double n) {
